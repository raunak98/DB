...
import { checkKerberosEncryptionType, getRequestItem } from '../helpers/utils'
...
export const getPendingJustification = async (pageSize, pageNumber) =>
  // Local Data
  // axios({
  //   url: '/api/metadata/justificationDataResponse',
  //   baseURL: 'http://localhost:8081'
  // })
  axios({
    url: `/v0/requests/pendingJustification?pageSize=${pageSize}&pageNumber=${pageNumber}`,
    method: 'get'
  }).then((res) => {
    const result = {}
    ...
    res?.data?.hits?.hits?.forEach((item) => {
      ...
      justificationData.push({
        ...
        requestType: item?._source?.requestType ? getRequestType(item?._source?.requestType) : '',
        ...
        requester: `${
          item._source?.decision?.actors?.active[0]?.givenName
            ? item._source?.decision?.actors?.active[0]?.givenName
            : ''
        } ${
          item._source?.decision?.actors?.active[0]?.sn
            ? item._source?.decision?.actors?.active[0]?.sn
            : ''
        }`,
        ...
        requestorID: item?._source?.requester?.id.split('/')[2],
        requestID: item?._source?.id,
        approverID: approverIds,
        common: item?._source?.request?.common,
        checked: false
      })
    })
    result.justificationData = justificationData
    return result
  })

// This API call required for future development and debugging
// export const getJustificationMetadata = async () => {
//   const response = await axios({
//     url: '/api/metadata/justificationTable',
//     baseURL: 'http://localhost:8081'
//   })
//     .then((res) => res.data)
...
export const getJustificationMetadata = async () => {
  const response = await axios(`/v0/configuration/metaType=justificationTable`)
    .then((res) => res.data)
    .catch((err) => console.error(err))
  return response
}

export const reassignItem = async (payload) => {
  const response = await axios({
    url: '/v0/requests/reassign',
    method: 'post',
    data: payload
  }).then((res) => res)
...
export const sortJustifications = (payload) =>
  axios({
    url: `/v0/sort/common`,
    data: payload,
    method: 'post'
...
export const searchJustification = (payload) =>
  axios({
...
export const getJustificationAccountSummaryStructure = async () => {
  const response = await axios({
    method: 'get',
...
export const getJustificationGroupSummaryStructure = async () => {
  const response = await axios({
    method: 'get',
...
export const getJustificationSummary = async (requestId) => {
  const response = await axios({
  ...
  response?.decision?.phases?.forEach((phase) => {
    ...
    phaseInfo.push({
      phaseId: seperateWordsWithCapitalLetter(phaseName),
      status: phase.status,
      decision: phase.decision ? phase.decision : '--',
  ...
  if (response?.request?.common?.groupDetails?.dbagExtensionAttribute6) {
    ...
    privateData = input.split('').map(Number)
  }
  ...
  return [
    ...
    {
      id: 'dbAGInfrastructureNARID',
      value: response?.request?.common?.groupDetails?.dbAGInfrastructureNARID
        ? response?.request?.common?.groupDetails?.dbAGInfrastructureNARID[0]
        : ''
    },
    {
      id: 'dbagSupportGroup',
...
export const getJustificationEntitlementSummaryStructure = async () => {
  const response = await axios({
    method: 'get',
