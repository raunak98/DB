...
import { checkApplicationAdmin, checkServiceDeskAdmin } from 'helpers/utils'
...
const Admin = () => {
  ...
  useEffect(async () => {
    ...
    await adminApi.getMetadata().then((res) => {
      setAccordionData(res)
    })
    dispatch(updateShowBigLoader(false))
    localStorage.setItem('component', 'Admin')
  }, [])

  const renderNavElemnt = (item, isChildElement) => {
    const hideItems =
      (checkApplicationAdmin(role) &&
        ['Service Desk Administration'].includes(translate(item.title))) ||
      (['AD Account Administration', 'AD Group Administration'].includes(translate(item.title)) &&
        checkServiceDeskAdmin(role))
    ...
    if ((checkApplicationAdmin(role) && checkServiceDeskAdmin(role)) || !hideItems) {
      return (
    ...
    <>
      ...
      <Breadcrumb
        path={[
          ...
          { label: translate('admin.header.title'), url: '' }
        ]}
      />
